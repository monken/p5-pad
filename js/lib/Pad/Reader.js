...
Pad.Reader = Ext.extend(Pad.Panel, {
    ...
    onActivate: function(tab, newtab, oldtab) {
        ...
        if(this.loaded) {
            Pad.UI.FilesPanel.add({author: this.author, release: this.release, xtype: 'padfiles' });
            Pad.UI.RelatedPanel.add({author: this.author, release: this.release, xtype: 'padrelated' });
        } else {
            Pad.UI.FilesPanel.onBeforeLoad();
            Pad.UI.RelatedPanel.onBeforeLoad();
        }
    },
    ...
    onLoad: function(res) {
        ...
        if(res) {
            ...
            Pad.UI.FilesPanel.add({author: this.author, release: this.release, xtype: 'padfiles', references: 1 }, true);
            Pad.UI.RelatedPanel.add({author: this.author, release: this.release, xtype: 'padrelated', references: 1 }, true);
